...
switch (service) {
	...
	case 'UserInfo':
		...
		break;
	case 'Dashboards':
        Dataset.dashboards = sqlLib.executeRecordSetObj("select * from metric2.m2_dashboard WHERE user_id = " + userid + " order by dashboard_id");
        output = JSON.stringify(Dataset);
        break;
	case 'Position':
	...
	case 'EditSettingsDialog':
        output = dialogLib.showSettingsDialog();
		break;
	...
	case 'RefreshWidget':
	    //Change 001
        //output = JSON.stringify(widgetLib.showWidgetDiv(dashboardwidgetid));
        //output = widgetLib.showWidgetContents(dashboardwidgetid,'');
        Dataset.widgets = widgetLib.showWidgetContents(dashboardwidgetid,'');
        Dataset.widgetData = widgetContents;
        output = JSON.stringify(Dataset);
		break;
	case 'Alerts':
