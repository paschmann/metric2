...
CREATE VIEW "METRIC2"."M2_WIDGET_BLOCKEDTRANS" ( "VALUE" ) AS SELECT COUNT(HOST) AS VALUE FROM SYS.M_BLOCKED_TRANSACTIONS WITH READ ONLY;
...
CREATE VIEW "METRIC2"."M2_WIDGET_SYS_MEM" ( "SM" ) AS SELECT round(sum(VALUE/1024/1024/1024),1) as SM FROM SYS.M_MEMORY WHERE NAME = 'SYSTEM_MEMORY_SIZE' WITH READ ONLY;
...
CREATE PROCEDURE METRIC2.M2_P_CLONEMETRIC(v_dwid INT, v_did INT) LANGUAGE SQLSCRIPT AS BEGIN INSERT INTO METRIC2.M2_DASHBOARD_WIDGET SELECT METRIC2.DASHBOARD_WIDGET_ID.NEXTVAL, :v_did, WIDGET_ID, TITLE, WIDTH, HEIGHT, ROW_POS, COL_POS, REFRESH_RATE FROM metric2.m2_dashboard_widget WHERE dashboard_widget_id  = :v_dwid; 
    INSERT INTO METRIC2.M2_DASHBOARD_WIDGET_PARAMS SELECT METRIC2.DASHBOARD_WIDGET_PARAM_ID.NEXTVAL, METRIC2.DASHBOARD_WIDGET_ID.CURRVAL, PARAM_ID, VALUE, WIDGET_ID, DT_ADDED FROM METRIC2.M2_DASHBOARD_WIDGET_PARAMS WHERE DASHBOARD_WIDGET_ID = :v_dwid; 
END;
CREATE PROCEDURE METRIC2.M2_P_CLONEDASHBOARD(v_did INT, v_userid INT) LANGUAGE SQLSCRIPT AS BEGIN 
    DECLARE v_dwid BIGINT; DECLARE v_vieworder INT; DECLARE v_newdid BIGINT DEFAULT 0; DECLARE CURSOR c_cursor1 FOR SELECT DASHBOARD_WIDGET_ID FROM M2_DASHBOARD_WIDGET WHERE DASHBOARD_ID = :v_did;
    SELECT METRIC2.DASHBOARD_ID.NEXTVAL INTO v_newdid from dummy;    
    SELECT (MAX(VIEWORDER) + 1) INTO v_vieworder from METRIC2.M2_DASHBOARD WHERE USER_ID = :v_userid;
    INSERT INTO METRIC2.M2_DASHBOARD SELECT :v_newdid, TITLE || ' Clone', SUBTITLE, DT_ADDED, USER_ID, REFRESH_RATE, WIDTH, HEIGHT, BG_COLOR, TV_MODE, :v_vieworder, '', BG_URL FROM metric2.m2_dashboard WHERE dashboard_id  = :v_did;
    FOR cur_row as c_cursor1 DO CALL METRIC2.M2_P_CLONEMETRIC(cur_row.DASHBOARD_WIDGET_ID, :v_newdid); END FOR;
END;

-- Create demo user
INSERT INTO METRIC2.M2_USERS (user_ID, name, lname, email_domain, email, password, acct_type, dt_added) VALUES (1, 'Demo', 'User', 'Lithium Labs', 'demo@metric2.com', '5749A96C37BEEF97ED177C17D625385E343399D3A1CD232BA4D59A1E5C142CAD', '0', '0123-01-01 00:00:00.0');
INSERT INTO METRIC2.M2_USERS (user_ID, name, lname, email_domain, email, password, acct_type, dt_added) VALUES (2, 'View', 'Only', 'Lithium Labs', 'view@metric2.com', '5749A96C37BEEF97ED177C17D625385E343399D3A1CD232BA4D59A1E5C142CAD', '1', '0123-01-01 00:00:00.0');
...
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (11,5,'TEXT1','Static','',300,1,'Any form of Static Text','Text','true',null,0, null);
...
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (38,10,'SQL1','SQL','SELECT VALUE FROM METRIC2.M2_WIDGET_BLOCKEDTRANS',200,0,'SQL Statement to fetch the current count of blocked transactions','SQL Query','false',null,1, 'VALUE');
...
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (130,34,'LINECOL','COLOR','#CCCCCC',600,0,'HTML Color: Color of the sparkline','Sparkline Color','true',null,0, null);
...
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (192,47,'HEXCOLOR1','COLOR','#DEEDF6',500,0,'Required, color in Hex (e.g #DEEDF6)','Min Color','true',2,0, null);
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (193,47,'HEXCOLOR2','COLOR','#2A89C1',500,0,'Required, color in Hex (e.g #2A89C1)','Max Color','true',2,0, null);
...
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (190,50,'HEXCOLOR1','COLOR','#DEEDF6',500,0,'Required, color in Hex (e.g #DEEDF6)','Min Color','true',2,0, null);
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (191,50,'HEXCOLOR2','COLOR','#2A89C1',500,0,'Required, color in Hex (e.g #2A89C1)','Max Color','true',2,0, null);

-- 51. Google API
INSERT INTO "METRIC2"."M2_WIDGET_PARAM" VALUES (194,51,'TEXT1','Static','',300,1,'Any form of Static Text','Text','true',null,0,null);
